    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      switch (type) {
        ...
        case 'minus':
          ...
          // 如果出现产品的个数小于等于1,传给服务器的个数为0（原封不动）
          disNum = cart.skuNum > 1 ? -1 : 0
          break
        case 'change':
          // 用户输入进来的最终量，非法的（带汉字|负数），带给服务器数字0
          if (isNaN(disNum) || disNum < 1) {
            disNum = 0
          } else {
            // 属于正常情况（小数：取整），带给服务器变化的量 ：用户输入最终的量 - 产品起始的数
      ...
      try {
        // 代表修改成功
        await this.$store.dispatch('addOrUpdataShopCart', { skuId: cart.skuId, skuNum: disNum })
        // 再一次获取服务器最新的数据进行展示
        this.getData()
      } catch (error) {
        alert(error.message)
